...
public class EndPointsFactoryImpl implements EndPointsFactory {
    ...
    private void addIx3EndPoints(List<EndPoint> endPoints) {
        ...
        endPoints.add(EndPoint.createEndPoint("/session", null, new BeanMapper(Usuario.class, "foto,claveValidacionEmail,lockedUntil,numFailedLogins,password,acl,memberOf,validadoEmail>,fechaUltimoAcceso>,fechaEnvioCorreoAvisoBorrarUsuario>", null)));
    }
    ...
    private void addSiteEndPoints(List<EndPoint> endPoints, String path) {
        endPoints.add(EndPoint.createEndPointCrud(path, new BeanMapper(Usuario.class, "foto,claveValidacionEmail,lockedUntil,numFailedLogins,password,acl,memberOf,validadoEmail>,fechaUltimoAcceso>,fechaEnvioCorreoAvisoBorrarUsuario>", null)));
        endPoints.add(EndPoint.createEndPoint(path + "/Usuario", "POST", new BeanMapper(Usuario.class, "foto,claveValidacionEmail,lockedUntil,numFailedLogins,<password,acl,memberOf,validadoEmail>,fechaUltimoAcceso>,fechaEnvioCorreoAvisoBorrarUsuario>", null)));
    ...
    private void addEmpresaEndPoints(List<EndPoint> endPoints, String path) {
        ...
        endPoints.add(EndPoint.createEndPointCrud(path, Empresa.class));
        endPoints.add(EndPoint.createEndPointCrud(path, new BeanMapper(Oferta.class, null, "ciclos")));
        endPoints.add(EndPoint.createEndPointCrud(path, new BeanMapper(Candidato.class, "usuario.foto,usuario.claveValidacionEmail,usuario.lockedUntil,usuario.numFailedLogins,usuario.password,usuario.acl,usuario.memberOf,usuario.validadoEmail>,usuario.tipoUsuario>,usuario.titulado.configuracion,usuario.fechaUltimoAcceso>,usuario.fechaEnvioCorreoAvisoBorrarUsuario>", null)));
